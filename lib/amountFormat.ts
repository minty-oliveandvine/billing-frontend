...
export function amountIntegerDigits(cleaned: string): number {
...
export function isWithinAmountLimits(raw: string): boolean {
  const cleaned = cleanAmountString(raw);
...
export function formatMoney(
  ...
  if (!amount) return "";
  const symbol = currencyLabel.trim();
...
export function toAmountEditString(raw: string | null | undefined): string {
